...
def run_and_create_demonstration(movie: retro.Movie, env : VecNormalize):
    ...
    json_dict = {'movie_obs':[], 'movie_rewards':[], 'movie_actions': [], 'movie_episodes':[], 'movie_returns':[]}
    ...
    while movie.step():
        ...
        json_dict['movie_actions'].append(keys)


        #print("Keys: ", keys)
        obs, rewards, dones, info = env.step(keys[0])

        json_dict['movie_obs'].append(obs)
        json_dict['movie_rewards'].append(rewards)
    
    
    json_dict['movie_episodes'] = [False*episode_counter]
    json_dict['movie_episodes'][0] = True
